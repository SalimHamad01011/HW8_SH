...
class rankineModel():
    def __init__(self):
        ...
        '''
        self.SI = True  # Ensure this attribute is defined
        self.p_low=None
        self.p_high=None
        self.t_high=None
        self.RC = None
...
class rankineController():
    def __init__(self, input_widgets, display_widgets, view):
        """
        ...
        :param input_widgets: a list of widgets for user input
        :param display_widgets: a list of widgets for displaying data
        :param view: instance of the View class
        """
        self.Model = rankineModel()  # Initialize the Model object
        self.View = view
        self.IW = input_widgets
        self.DW = display_widgets
        self.View.setWidgets(*self.IW, *self.DW)
        self.buildVaporDomeData()

    def setView(self, view_instance):
        """
        Set the view instance
        :param view_instance: The instance of the view
        """
    ...
    def buildVaporDomeData(self, nPoints=500):
        ...
        if self.View is None:
            print("View is not initialized. Please set the view instance before building vapor dome data.")
            return
        ...
        for p in P:
            ...
            self.View.satLiqPlotData.addPt((sat.tsat, p, sat.uf, sat.hf, sat.sf, sat.vf))
            self.View.satVapPlotData.addPt((sat.tsat, p, sat.uf, sat.hg, sat.sg, sat.vg))
        self.View.satLiqPlotData.addPt((critProps.t, critProps.p, critProps.u, critProps.h, critProps.s, critProps.v))
        self.View.satVapPlotData.addPt((critProps.t, critProps.p, critProps.u, critProps.h, critProps.s, critProps.v))
    ...
    def updateModel(self):
        ...
        """
        try:
            # Check if le_PHigh has text entered
            p_high_text = self.View.le_PHigh.text().strip()
            if p_high_text != '':
                # Convert text to float
                self.Model.p_high = float(p_high_text) * PCF  # get the high pressure isobar in kPa
            else:
                # Handle case when le_PHigh is empty
                # You can raise an error, show a message, or handle it according to your application's logic
                raise ValueError("Please enter a value for high pressure.")
        except ValueError as e:
            # Handle the ValueError (e.g., show a message box with the error)
            print("Error:", e)
    ...
    def buildVaporDomeData(self, nPoints=500):
        ...
        """
        if self.Model is None:
            # Handle the case when self.Model is not initialized
            # Example:
            # print("Model is not initialized. Please initialize it before building vapor dome data.")
            return
